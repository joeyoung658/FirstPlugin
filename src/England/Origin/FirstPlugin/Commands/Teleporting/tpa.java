...
//            }
            ...
            if (args.length == 1) {
                ...
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&' , "&e[&4Server&e]&f ") +ChatColor.AQUA + "You have requested to teleport to " + target.getDisplayName());
                target.sendMessage(((Player) sender).getDisplayName() +  ChatColor.AQUA +  " has requested to teleport to you!"  + ChatColor.RED + " Type /tpyes to accept the request.");
                if (tparequests.containsKey(target)) {
                    tparequests.remove(target);
                    tparequests.put(target, ((Player) sender));
                } else {
                    tparequests.put(target, ((Player) sender));
                }
            } else {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&' , "&e[&4Server&e]&f ") +ChatColor.RED + "Incorrect arguments! /tpa [PlayerName]");
            }
        }
        return false;
    }
